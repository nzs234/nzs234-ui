...
async function main() {
  ...
  // 1. Click Next on type step (selects default lora)
  ...
  // 2. Click Next on model step (selects default sdxl-lora)
  await send('Runtime.evaluate', {
    expression: `(() => {
      const nextBtn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('下一步'));
      if (nextBtn) nextBtn.click();
  ...
  // 4. Now on files step -> click Next to trigger validation error alert and invalid inputs
  await send('Runtime.evaluate', {
    expression: `(() => {
      const nextBtn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('下一步'));
      if (nextBtn) nextBtn.click();
  ...
  const evalRes = await send('Runtime.evaluate', {
    expression: `(() => {
      ...
      return {
        stepTitle: document.querySelector('h1')?.innerText,
