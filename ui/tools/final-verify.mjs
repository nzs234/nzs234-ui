...
async function main() {
  ...
  for (const vp of viewports) {
    ...
    for (const p of pagesList) {
      ...
      console.log(`[${vp.name}] /${p}: scrollWidth=${data.docScrollWidth}, clientWidth=${data.docClientWidth}, ok=${data.ok}`);
  ...
  console.log('Generate @320 Region rows measurement:', JSON.stringify(regCheck.result?.value, null, 2));
  ...
  for (const theme of themes) {
    for (const p of pagesList) {
      ...
      const data = contrastRes.result?.value;
      console.log(`[Theme: ${theme}] /${p} Contrast Non-large Fails: ${data.failCount}`);
    }
  }
