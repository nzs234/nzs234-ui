...
export function validateConfig(config: Record<string, any>): ValidationResult {
  ...
  // ========== 冲突 4: 数据集路径为空 ==========
  const trainDataDir = String(config.train_data_dir || '').trim()

  if (!trainDataDir) {
    errors.push({
      message: '训练数据集路径为空，无法开始训练。',
      fields: ['train_data_dir']
    })
  }

  // ========== 冲突 5: 模型路径为空 ==========
  const modelPath = String(config.pretrained_model_name_or_path || '').trim()

  if (!modelPath) {
    errors.push({
      message: '预训练模型路径为空，无法开始训练。',
      fields: ['pretrained_model_name_or_path']
    })
  }
